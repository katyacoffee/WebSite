...
base_dir_serv_k_ind = '~mikhnevo/K-INDEX'
source_vlf = 'vlf'
source_tec = 'tec'
source_gps = 'gps'
source_lem = 'lem'
...
def get_img_k_index_for_day(img_list: list[str], year, mon, day) -> list[str]:
    ...
    for im in img_list:
        ...
        if int(s[2]) == int(year) and int(s[3]) == int(mon) and int(s[4]) == int(day):
            #img_list_k.append(im)
            #TODO: помещаем две предыдущих из img_list + im + две следующих из img_list в список img_list_k
...
def get_img_list(yr: str, mon: str, day: str, source: str) -> list[str]:
    ...
    try:
        ...
        if source == source_vlf:
            ...
            img_list = get_img_k_index(img_list, yr, mon) #TODO: тут надо брать из get_img_k_index_for_day()
            ...
            for pic in img_list: # TODO: убрать
                if get_day_from_k_pic(pic) == int(day):
                    ...
                    no_data = False
                    break
            if no_data: # TODO: а тут наоборот условие
                img_list = []
